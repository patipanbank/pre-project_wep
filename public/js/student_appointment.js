...
if (detail) {
    try {
        ...
        if (result.success) {
            ...
            if (selectedDates.length > 0) {
                // ใช้ localISOString เพื่อให้แน่ใจว่าวันที่ถูกต้อง
                const firstDate = new Date(selectedDates[0].getTime() - (selectedDates[0].getTimezoneOffset() * 60000))
                    .toISOString().split('T')[0];
                const lastDate = new Date(selectedDates[selectedDates.length - 1].getTime() - 
